cfci <- function(df, dataType = 0, numCategoriesToDiscretize = 4, depth = 3, alpha = 0.05, 
	...
	if(numBootstrap < 1){
		...
		algorithm <- .jnew("edu/cmu/tetrad/algcomparison/algorithm/oracle/pag/Cfci", indTest)
		...
		# Initiate Bootstrapping CFCI
		cfci_instance <- .jnew("edu/pitt/dbmi/algo/bootstrap/GeneralBootstrapTest", tetradData, algorithm, numBootstrap)
		edgeEnsemble <- .jfield("edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble", name=ensembleMethod)
		cfci_instance$setEdgeEnsemble(edgeEnsemble)
		cfci_instance$setParameters(parameters_instance)
	}

    cfci_instance$setVerbose(verbose)
