gfci.mixed <- function(df, numCategoriesToDiscretize = 4, penaltyDiscount = 4,
    structurePrior = 1.0, maxDegree = 3, maxPathLength = -1, alpha = 0.05,
	...
	if(numBootstrap < 1){
    	...
    	indTest <- .jnew("edu/cmu/tetrad/search/IndTestConditionalGaussianLRT", tetradData, alpha)
    
		indTest <- .jcast(indTest, "edu/cmu/tetrad/search/IndependenceTest")

    	# Data Frame to Tetrad Dataset
    	score <- dataFrame2TetradConditionalGaussianScore(df, numCategoriesToDiscretize,
