pc <- function(df, dataType = 0, numCategoriesToDiscretize = 4, depth = 3, alpha = 0.05,
	...
	if(numBootstrap < 1){
		...
		algorithm <- .jnew("edu/cmu/tetrad/algcomparison/algorithm/oracle/pattern/Pc", indTest)
		...
		pc_instance <- .jnew("edu/pitt/dbmi/algo/bootstrap/GeneralBootstrapTest", tetradData, algorithm, numBootstrap)
		edgeEnsemble <- .jfield("edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble", name=ensembleMethod)
		pc_instance$setEdgeEnsemble(edgeEnsemble)
		pc_instance$setParameters(parameters_instance)
	}
	
    pc_instance$setVerbose(verbose)
