...
public final class TetradCmd {
		...
		while (tokenizer.hasToken()) {
			...
			if ("-data".equalsIgnoreCase(token)) {
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("'-data' tag must be followed "
							+ "by an argument indicating the path to the data " + "file.");
				}
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException(
							"'-datatype' tag must be followed " + "by either 'discrete' or 'continuous'.");
				}
				...
				try {
					...
					if (this.depth < -1) {
						throw new IllegalArgumentException(
								"'depth' must be followed " + "by an integer >= -1 (-1 means unlimited).");
					}
				...
				try {
					...
					throw new IllegalArgumentException(
							"'-significance' must be " + "followed by a number in the range [0.0, 1.0].");
				}
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("'-outfile' tag must be "
							+ "followed  by an argument indicating the path to the " + "data file.");
				}
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("-numNodes must be followed by an integer >= 3.");
				}
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("-numEdges must be followed by an integer >= 0.");
				}
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("'-knowledge' tag must be followed "
							+ "by an argument indicating the path to the knowledge " + "file.");
				}
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("'-testType' tag must be followed by 'delta' or 'wishart'");
				}
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("'-graphxml' tag must be followed "
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("'-graphtxt' tag must be followed "
				...
				if (argument.startsWith("-")) {
					throw new IllegalArgumentException("'-initialgraphtxt' tag must be followed "
							+ "by an argument indicating the path to the file where the graph txt output "
							+ "is to be written.");
				}
				...
				try {
					...
					if (argument.startsWith("-")) {
						throw new IllegalArgumentException("'-sampleprior' tag must be followed "
				...
				try {
					...
					if (argument.startsWith("-")) {
						throw new IllegalArgumentException("'-structureprior' tag must be followed "
				...
				try {
					...
					if (argument.startsWith("-")) {
						throw new IllegalArgumentException("'-penaltydiscount' tag must be followed "
				...
				try {
					...
					if (argument.startsWith("-")) {
						throw new IllegalArgumentException("'-bootstrap' tag must be followed "
					...
					if (numBootstrapSamples < 0) {
						throw new IllegalArgumentException("Bootstrap sampling number must be > 0.");
					}
				} catch (NumberFormatException e) {
					throw new IllegalArgumentException("Bootstrap sampling parameter is not a number.");
				}
			} else {
		...
		try {
			try {
				...
				if (useCovariance) {
					...
					outPrint("# variables = " + data.getNumColumns() + ", # cases = " + data.getNumRows());
					this.data = data;
				}
	...
	private void printRandomDag() {
		...
		do {
			dag = new Dag(GraphUtils.randomGraph(nodes, 0, _numEdges, 30, 15, 15, false));
	...
	private void runBootstrap() {
		if (this.covarianceMatrix != null) {
			throw new IllegalStateException("Cannot run bootstrap on the covariance Data.");
		}
		...
		if ("fges".equalsIgnoreCase(algorithmName)) {
			...
			throw new IllegalStateException(
					"Bootstrap Search does not support the " + algorithmName + " algorithm yet.");
		}

		BootstrapTest bootstrapTest = new BootstrapTest(data, algName, numBootstrapSamples);
	...
	private void runFofc() {
		...
		if (this.data != null) {
			fofc = new FindOneFactorClusters(this.data, this.testType, FindOneFactorClusters.Algorithm.GAP,
					significance);
			if (!this.data.isContinuous()) {
				outPrint("Please supply continuous data.");
			}
		} else if (this.covarianceMatrix != null) {
			fofc = new FindOneFactorClusters(this.covarianceMatrix, this.testType, FindOneFactorClusters.Algorithm.GAP,
					significance);
		} else {
			throw new IllegalStateException("Data did not load correctly.");
		}
