...
public final class GraphUtils {
	...
	public static void kamadaKawaiLayout(Graph graph, boolean randomlyInitialized, double naturalEdgeLength,
	 ...
	 */
	private static int getMeasurementModel(Graph graph, List<Node> latents, List<List<Node>> partition) {
		int totalSize = 0;
	...
	public static Graph randomDag(int numNodes, int numLatentConfounders, int maxNumEdges, int maxDegree,
	...
	public static Dag randomDag(List<Node> nodes, int numLatentConfounders, int maxNumEdges, int maxDegree,
			int maxIndegree, int maxOutdegree, boolean connected) {
		return new Dag(
				randomGraph(nodes, numLatentConfounders, maxNumEdges, maxDegree, maxIndegree, maxOutdegree, connected));
	}

	public static Graph randomGraph(int numNodes, int numLatentConfounders, int maxNumEdges, int maxDegree,
			int maxIndegree, int maxOutdegree, boolean connected) {
		List<Node> nodes = new ArrayList<>();
	...
	public static Graph randomGraph(List<Node> nodes, int numLatentConfounders, int maxNumEdges, int maxDegree,
		...
		// It is still unclear whether we should use the random forward edges
		// method or the
		...
		// return randomGraphUniform(nodes, numLatentConfounders, maxNumEdges,
		// maxDegree, maxIndegree, maxOutdegree, connected);
	}
	 ...
	 */
	public static Graph randomGraphUniform(int numNodes, int numLatentConfounders, int maxNumEdges, int maxDegree,
			int maxIndegree, int maxOutdegree, boolean connected) {
		List<Node> nodes = new ArrayList<>();
		...
		return randomGraphUniform(nodes, numLatentConfounders, maxNumEdges, maxDegree, maxIndegree, maxOutdegree,
				connected);
	}
	...
	public static Graph randomGraphUniform(List<Node> nodes, int numLatentConfounders, int maxNumEdges, int maxDegree,
		...
		if (maxNumEdges < 0 || maxNumEdges > numNodes * (numNodes - 1)) {
			throw new IllegalArgumentException(
					"NumEdges must be " + "at least 0 and at most (#nodes)(#nodes - 1) / 2: " + maxNumEdges);
		}

		if (numLatentConfounders < 0 || numLatentConfounders > numNodes) {
			throw new IllegalArgumentException("Max # latent confounders must be "
					+ "at least 0 and at most the number of nodes: " + numLatentConfounders);
		}
	...
	public static Graph randomGraphRandomForwardEdges(int numNodes, int numLatentConfounders, int numEdges,
			int maxDegree, int maxIndegree, int maxOutdegree, boolean connected) {
	...
	public static Graph randomGraphRandomForwardEdges(List<Node> nodes, int numLatentConfounders, int numEdges,
			int maxDegree, int maxIndegree, int maxOutdegree, boolean connected, boolean layoutAsCircle) {
		if (nodes.size() <= 0) {
			throw new IllegalArgumentException("NumNodes most be > 0: " + nodes.size());
		}
		...
		if (numEdges < 0 || numEdges > size * (size - 1)) {
			throw new IllegalArgumentException(
					"numEdges must be " + "greater than 0 and <= (#nodes)(#nodes - 1) / 2: " + numEdges);
		}

		if (numLatentConfounders < 0 || numLatentConfounders > nodes.size()) {
			throw new IllegalArgumentException("MaxNumLatents must be "
					+ "greater than 0 and less than the number of nodes: " + numLatentConfounders);
		}
	...
	public static Graph scaleFreeGraph(int numNodes, int numLatentConfounders, double alpha, double beta,
	...
	private static Graph scaleFreeGraph(List<Node> _nodes, int numLatentConfounders, double alpha, double beta,
			double delta_in, double delta_out) {

		if (alpha + beta >= 1) {
			throw new IllegalArgumentException(
					"For the Bollobas et al. algorithm," + "\napha + beta + gamma = 1, so alpha + beta must be < 1.");
		}

		// System.out.println("# nodes = " + _nodes.size() + " latents = " +
		// numLatentConfounders
		// + " alpha = " + alpha + " beta = " + beta + " delta_in = " + delta_in
		// + " delta_out = " + delta_out);
	 ...
	 *
	 * @param dag
	 *            A Dag returned from any of the randomDag methods
	 * @param maxNumCycles
	 *            Algorithm will add at most this many cyclic edges to the graph
	 * @param minCycleLength
	 *            The smallest number of edges allowed for creating cycles
	 */
	public static Graph addCycles(Graph dag, int maxNumCycles, int minCycleLength) {

		if (maxNumCycles <= 0) {
			throw new IllegalArgumentException("maxNumCycles most be > 0: " + maxNumCycles);
		}
	...
	public static Graph cyclicGraph2(int numNodes, int numEdges, int maxDegree) {
		...
		LOOP: while (graph.getEdges().size() < numEdges /* && ++count1 < 100 */) {
			...
			// Make sure you won't created any two cycles (this will be done
			// later, explicitly)
	...
	public static Node getAssociatedNode(Node errorNode, Graph graph) {
		...
		if (children.size() != 1) {
			...
			throw new IllegalArgumentException(
					"An error node should have only " + "one child, which is its associated node: " + errorNode);
		}
	 ...
	 *
	 * @param target
	 *            a node in the given DAG.
	 * @param dag
	 *            the DAG with respect to which a Markov blanket DAG is to to be
	 *            calculated. All of the nodes and edges of the Markov Blanket
	 *            DAG are in this DAG.
	 */
	public static Dag markovBlanketDag(Node target, Graph dag) {
		...
		for (Object parent2 : parents) {
			...
			for (Object aParentsOfChildren : parentsOfChildren) {
				...
				if (edge1 != null && edge2 == null) {
					Edge newEdge = new Edge(parent, parentOfChild, edge1.getProximalEndpoint(parent),
		...
		for (Object aChildren1 : children) {
			...
			for (Object aParentsOfChildren : parentsOfChildren) {
				...
				if (edge1 != null && edge2 == null) {
					Edge newEdge = new Edge(child, parentOfChild, edge1.getProximalEndpoint(child),
							edge1.getProximalEndpoint(parentOfChild));
	...
	/**
	 * @param graph
	 *            The graph in which a directed cycle is sought.
	...
	/**
	 * @param graph
	 *            The graph in which a directed path is sought.
	 * @param node1
	 *            The 'from' node.
	 * @param node2
	 *            The 'to'node.
	...
	// all adjancencies are directed <=> there is no uncertainty about who the
	// parents of 'node' are.
	...
	private static void directedPathsFromToVisit(Graph graph, Node node1, Node node2, LinkedList<Node> path,
			List<List<Node>> paths, int maxLength) {
	...
	private static void semidirectedPathsFromToVisit(Graph graph, Node node1, Node node2, LinkedList<Node> path,
			List<List<Node>> paths, int maxLength) {
		if (maxLength != -1 && path.size() > maxLength - 2) {
			return;
		}
	...
	private static void allPathsFromToVisit(Graph graph, Node node1, Node node2, LinkedList<Node> path,
			List<List<Node>> paths, int maxLength) {
		path.addLast(node1);
	...
	private static void allDirectedPathsFromToVisit(Graph graph, Node node1, Node node2, LinkedList<Node> path,
			List<List<Node>> paths, int maxLength) {
		path.addLast(node1);
	...
	private static void treksIncludingBidirected(SemGraph graph, Node node1, Node node2, LinkedList<Node> path,
	...
	public static List<Edge> adjacenciesComplement(Graph graph1, Graph graph2) {
		...
		for (Edge edge1 : graph1.getEdges()) {
			...
			// if (node21 == null) {
			// continue;
			//// throw new IllegalArgumentException("There was no node by that
			// name in the reference graph: " + name1);
			// }
			//
			// if (node22 == null) {
			// continue;
			//// throw new IllegalArgumentException("There was no node by that
			// name in the reference graph: " + name2);
			// }
	 ...
	 * @param newVariables
	 *            The new variables to use, with the same names as the old ones.
	...
	public static Graph replaceNodes(Graph originalGraph, List<Node> newVariables) {
		...
		for (Triple triple : originalGraph.getUnderLines()) {
			convertedGraph.addUnderlineTriple(convertedGraph.getNode(triple.getX().getName()),
					convertedGraph.getNode(triple.getY().getName()), convertedGraph.getNode(triple.getZ().getName()));
		}

		for (Triple triple : originalGraph.getDottedUnderlines()) {
			convertedGraph.addDottedUnderlineTriple(convertedGraph.getNode(triple.getX().getName()),
					convertedGraph.getNode(triple.getY().getName()), convertedGraph.getNode(triple.getZ().getName()));
		}

		for (Triple triple : originalGraph.getAmbiguousTriples()) {
			convertedGraph.addAmbiguousTriple(convertedGraph.getNode(triple.getX().getName()),
					convertedGraph.getNode(triple.getY().getName()), convertedGraph.getNode(triple.getZ().getName()));
		}
	 ...
	 *
	 * @param originalNodes
	 *            The list of nodes to be converted.
	 * @param newNodes
	 *            A list of new nodes, containing as a subset nodes with the
	 *            same names as those in <code>originalNodes</code>. the old
	 *            ones.
	...
	public static int getNumCorrectArrowpts(Graph correct, Graph estimated) {
		...
		for (Edge estEdge : edges) {
			...
			if (estEdge.getProximalEndpoint(estEdge.getNode1()) == Endpoint.ARROW
					&& correctEdge.getProximalEndpoint(estEdge.getNode1()) == Endpoint.ARROW) {
				numCorrect++;
			}

			if (estEdge.getProximalEndpoint(estEdge.getNode2()) == Endpoint.ARROW
					&& correctEdge.getProximalEndpoint(estEdge.getNode2()) == Endpoint.ARROW) {
				numCorrect++;
			}
		}
	 ...
	 *            The list of nodes to be converted.
	 * @param graph
	 *            A graph to be used as a source of new nodes.
	 * @return A new, converted, graph.
	 */
	...
	public static String graphToDot(Graph graph) {
		...
		for (Edge edge : graph.getEdges()) {
			builder.append(" \"").append(edge.getNode1()).append("\" -> \"").append(edge.getNode2())
					.append("\" [arrowtail=");
	...
	public static Graph parseGraphXml(Element graphElement, Map<String, Node> nodes) throws ParsingException {
		...
		if (!("variables".equals(graphElement.getChildElements().get(0).getLocalName()))) {
			throw new ParsingException(
					"Expecting variables element: " + graphElement.getChildElements().get(0).getLocalName());
		}
	 ...
	 * @param file
	 *            The file to save it in.
	 * @param xml
	 *            True if to be saved in XML, false if in text.
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if <code>graph</code> is not a directed graph.
	 */
	...
	private static boolean existsDirectedPathVisit(Node node1, Node node2, LinkedList<Node> path, int depth,
			Graph graph) {
		path.addLast(node1);
	 ...
	 *         another. Because the path needs to be non-empty, this can
	 *         distinguish cycles. The case where from = to but there is no
	 *         cycle from from to to needs to be checked separately.
	 */
	...
	private static boolean existsSemiDirectedPathVisit(Node node1, Node node2, LinkedList<Node> path, int maxDepth,
		...
		for (Edge edge : graph.getEdges(graph.getNode(node1.getName()))) {
			...
			// if (previous != null && graph.isAmbiguousTriple(previous, node1,
			// child)) {
			// continue;
			// }
	 ...
	 *
	 * @param indices
	 *            The indices of the desired nodes in <code>nodes</code>.
	 * @param nodes
	 *            The list of nodes from which we select a sublist.
	...
	private static StringBuilder undirectedEdges(List<Graph> graphs) {
		...
		for (int i = 0; i < graphs.size(); i++) {
			Graph graph = replaceNodes(undirectedGraphs.get(i), nodes);
	...
	private static StringBuilder directedEdges(List<Graph> directedGraphs) {
		...
		for (Graph directedGraph : directedGraphs) {
			Graph graph = replaceNodes(directedGraph, nodes);
		...
		for (int i = groups.size() - 1; i >= 0; i--) {
			b.append("\n\nUncontradicted in ").append(i + 1).append(" graph").append((i > 0) ? "s" : "")
					.append("...\n");
		...
		for (Edge edge : contradicted) {
			b.append("\n").append(index++).append(". ").append(Edges.undirectedEdge(edge.getNode1(), edge.getNode2()))
					.append(" (--> ").append(directionCounts.get(edge)).append(" <-- ")
	...
	public static String edgeMisclassifications(double[][] counts, NumberFormat nf) {
		...
		for (int i = 0; i < counts.length; i++) {
			for (int j = 0; j < counts[0].length - 1; j++) {
				if ((i == 0 && j == 0) || (i == 1 && j == 1) || (i == 2 && j == 2) || (i == 4 && j == 3)
						|| (i == 6 && j == 4)) {
		...
		builder.append("\nRatio correct edges to estimated edges = ")
				.append(nf2.format((correctEdges / (double) estimatedEdges)));
	...
	// Returns true if a path consisting of undirected and directed edges toward
	// 'to' exists of
	...
	public static String graphToText(Graph graph) {
		...
		Set<Triple> ambiguousTriples = graph.getAmbiguousTriples();
		if (!ambiguousTriples.isEmpty()) {
			fmt.format("%n%n%s", triplesToText(ambiguousTriples,
	...
	public static String graphNodesToText(Graph graph, String title, char delimiter) {
		StringBuilder sb = (title == null || title.length() == 0) ? new StringBuilder()
	...
	public static class GraphComparison {
		...
		public GraphComparison(int adjFn, int adjFp, int adjCorrect, int arrowptFn, int arrowptFp, int arrowptCorrect,
				double adjPrec, double adjRec, double arrowptPrec, double arrowptRec, int shd, int twoCycleCorrect,
				int twoCycleFn, int twoCycleFp, List<Edge> edgesAdded, List<Edge> edgesRemoved,
				List<Edge> edgesReorientedFrom, List<Edge> edgesReorientedTo, int[][] counts) {
			this.adjFn = adjFn;
			this.adjFp = adjFp;
	...
	public static TwoCycleErrors getTwoCycleErrors(Graph trueGraph, Graph estGraph) {
		...
		TwoCycleErrors twoCycleErrors = new TwoCycleErrors(adjCorrect, adjFn, adjFp);
	...
	public static class TwoCycleErrors {
		...
		public String toString() {
			String buf = "2c cor = " + twoCycCor + "\t" + "2c fn = " + twoCycFn + "\t" + "2c fp = " + twoCycFp;
	...
	private static boolean isDConnectedToVisit2(Node a, Node b, Node y, LinkedList<Node> path, List<Node> z,
			Graph graph) {
		if (b == y) {
			path.addLast(b);
			return true;
	...
	private static boolean reachable(Edge e1, Edge e2, Node a, List<Node> z, Graph graph) {
		...
		// System.out.println("Ancestors of " + z + " are " + zAncestors(z,
		// graph));
		// }
		return collider && ancestor;
	}
	...
	private static boolean existOnePathWithPossibleParents(Map<Node, List<Node>> previous, Node w, Node x, Node b,
			Graph graph) {
		if (w == x) {
			return true;
		}
		...
		for (Node r : p) {
			...
			if ((existsSemidirectedPath(r, x, graph)) || existsSemidirectedPath(r, b, graph)) {
	...
	// 'to' exists of
	// length at most 'bound'. Cycle checker in other words.
	public static List<Node> existsUnblockedSemiDirectedPath(Node from, Node to, Set<Node> cond, int bound,
			Graph graph) {
		Queue<Node> Q = new LinkedList<>();
		Set<Node> V = new HashSet<>();
		Q.offer(from);
